<template>
    <div class="container max-w-7xl mx-auto text-gray-900">

        <h3 class="text-3xl md:text-5xl font-medium mb-4">Perguntas frequentes</h3>

        <p class="text-lg mb-12 font-light">
            Se não encontrar o que procura aqui, consulte nosso time ou bot virtual.
        </p>

        <div class="grid grid-cols-1 md:grid-cols-3 md:gap-10">
            
            <div class="mb-12">
                <header class="mb-4 text-lg font-medium">
                    E se estiver sem internet, irei receber a chamada do interfone ou portaria?
                </header>

                <p class="mb-8 font-light">
                    Sim, o nosso sistema identifica que não há nenhum equipamento online e faz uma chamada telefônica
                    convencional.
                    Assim você não irá perder nenhuma chamada!
                </p>

                <header class="mb-4 text-lg font-medium">
                    Posso fazer ligações entre moradores do condomínio?
                </header>

                <p class="font-light">
                    Claro! Basta abrir o catálogo no aplicativo e escolher o apartamento no qual deseja chamar.
                </p>
            </div>

            <div class="mb-12">
                <header class="mb-4 text-lg font-medium">
                    Em caso de festa em meu apartamento, posso enviar convites aos meus amigos para entrada no condomínio via WhatsApp?
                </header>

                <p class="font-light">
                    Sim! O nosso sistema proporciona a opção de enviar convites via WhatsApp, assim seu convidado entrará rapidamente, evitando filas e com toda segurança que só o sistema Atendeaí pode proporcionar!
                </p>
            </div>

            <div class="mb-12">
                <header class="mb-4 text-lg font-medium">
                    Consigo ver a imagem de quem tocou o interfone mesmo antes de atender?
                </header>

                <p class="font-light">
                    Quando alguém chama no interfone, de imediato o seu app abre e exibe a imagem da portaria, assim você poderá visualizar e decide se irá atender ou não a chamada.
                </p>
            </div>
        </div>
    </div>
</template>